...
exports.processSpeech = async (req, res) => {
    ...
    try {
      ...
      //     model: 'deepseek-chat',
      //     messages: [
      //       { role: 'system', content: 'You are a helpful AI assistant for a clinic. Keep replies natural and short.' },
      ...
      // 🎤 Convert response to speech using ElevenLabs or Polly
      const audioBuffer = await elevenlabsService.textToSpeech(aiResponse);
  
      // 🔊 Serve audio as base64 inline
      ...
      res.type('text/xml').send(twiml.toString());
        ...
        console.error('❌ Error in processSpeech:', error.response?.data || error.message);
        twiml.say("Sorry, I'm having trouble right now. Let's try again.");
    //   twiml.redirect('/api/v1/voice');
        twiml.hangup()
        res.type('text/xml').send(twiml.toString());
    }
